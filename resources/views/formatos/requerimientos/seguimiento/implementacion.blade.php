<div class="table-responsive">
  <table class="table">
    ...
    <tbody id="searchable">
      @foreach ($implementacion as $registro)
        <tr id="{{$registro->folio}}"
              ...
              @foreach ($pausa as $pospuesto)
                @if ($registro->folio == $pospuesto->folio)
                  @if ($pospuesto->pausa == 2)
                    <button type="button" class="w-100 btn btn-rounded text-dark" data-bs-toggle="modal" data-bs-target="#esti{{$loop->parent->iteration}}">
          ...
          <div id="esti{{$loop->iteration}}" class="modal fade" tabindex="-1" aria-hidden="true">
                  ...
                  @foreach($pausa as $p)
                    ...
                    <div class="dropdown-menu dropdown-menu-lg-end" aria-labelledby="btnGroupVerticalDrop1">
                      @if ($p->pausa == 2 && $p->folio == $registro->folio)
                        ...
                        <a class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#confi{{$loop->parent->iteration}}">CANCELAR</a>
                      @else
          ...
          <div id="confi{{$loop->iteration}}" class="modal fade" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-body">
                    @foreach($pausa as $p)
